...
def null_cond_c(t, x, y, z, vx, vy, vz):
    x0, x1, x2, x3 = X0(t, x, y, z), X1(t, x, y, z), X2(t, x, y, z), X3(t, x, y, z)
    return x1*(x1*(x**2 + y**2)*(x1 - 1.0)*(vx*x*z + vy*y*z - vz*(x**2 + y**2))**2 + x1*(x1 - 1.0)*(vx*y - vy*x)**2*(x**2 + y**2 + z**2)**2*numpy.sin(x2)**2 + (x**2 + y**2)**2*(x**2 + y**2 + z**2)*(vx*x + vy*y + vz*z)**2)/((x**2 + y**2)**2*(x1 - 1.0)*(x**2 + y**2 + z**2)**2)
