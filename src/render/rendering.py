...
@njit
def rk4_step(t, y, h, settings:RenderSettings, grid:KDTreeType): # h is a step size
    k1 = GravMethod.geodesic_eq(t, y, grid, settings.Gamma_grid)
    k2 = GravMethod.geodesic_eq(t+h/2, y+h*k1/2, grid, settings.Gamma_grid)
    k3 = GravMethod.geodesic_eq(t+h/2, y+h*k2/2, grid, settings.Gamma_grid)
    k4 = GravMethod.geodesic_eq(t+h, y+h*k3, grid, settings.Gamma_grid)
    return y + (h/6) * (k1 + 2*k2 + 2*k3 + k4)

@njit
def integrator(settings:RenderSettings, y0:float, grid:KDTreeType, max_t:float=1000,
    ...
    while t < max_t:
        ...
        if GravMethod.singularity(y, grid, settings.g_grid) > threshold:
            return y, "singularity", settings.scene[0]

        # Adapt step size to how close the point is to any object
        speed = np.linalg.norm(GravMethod.geodesic_eq(t, y, grid, settings.Gamma_grid))
        min_dist = min(dists)
        h = min(h, 2*min_dist/speed)

        # Perform integration
        if t + h > max_t: h = max_t - t # Prevents overstepping
        y_coarse = rk4_step(t, y, h, settings, grid) # Coarse step
        y_mid = rk4_step(t, y, h/2, settings, grid)
        y_fine = rk4_step(t+h/2, y_mid, h/2, settings, grid) # Fine step made of 2 substeps
        # Error estimate
...
@njit
def trace(pos:Vec, dir:Vec, settings:RenderSettings, grid:KDTreeType): # Returns color the light ray starting from pos in dir 'sees'
    dir.is_normal()
    col = Vec(0,0,0)
    ...
    hit_pt, message, obj = integrator(settings, y0, grid, max_t=1000, threshold=1e3)
...
def first_render(settings:RenderSettings, grid:KDTreeType, pbar:ProgressBar): # First render
    ...
    for i in prange(settings.w * settings.h):
        ...
        col = trace(settings.cam_pos, ray_dir, settings, grid)
...
def render_seq(settings:RenderSettings):
    with ProgressBar(total=settings.w*settings.h) as pbar:
        img = first_render(settings, KDTree(settings.grid, compact=True), pbar)
